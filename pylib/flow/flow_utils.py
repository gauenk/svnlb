...
def visulize_flow_file(flow_filename, save_dir=None):
    ...
    if save_dir:
        ...
        cv2.imsave(os.path.join(save_dir, "%s-vis.png" % flow_filename[idx:-4]), img)


def flow2burst(flow_data):
    burst = []
    for t in range(flow_data.shape[0]):
        flow = rearrange(flow_data[t],'c h w -> h w c')
        img = flow2img(flow)
        burst.append(img)
