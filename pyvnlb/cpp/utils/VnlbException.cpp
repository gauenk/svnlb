...
#include <pyvnlb/cpp/utils/VnlbException.h>
#include <sstream>
...
VnlbException::VnlbException(const std::string& m) : msg(m) {}

VnlbException::VnlbException(
        const std::string& m,
        const char* funcName,
        const char* file,
        int line) {
...
const char* VnlbException::what() const noexcept {
    return msg.c_str();
}
