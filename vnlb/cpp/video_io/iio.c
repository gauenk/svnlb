...
#include <vnlb/cpp/utils/xmalloc.h>
...
static void *convert_data(void *src, int n, int dest_fmt, int src_fmt)
{
	...
	char *r = (char*)xmalloc(n * dest_width);
...
static void iio_hacky_uncolorize(struct iio_image *x)
{
	...
	case IIO_TYPE_UINT8: {
		uint8_t (*xd)[3] = x->data;
		uint8_t *r = (uint8_t*)xmalloc(n*sizeof*r);
		FORI(n)
	...
	case IIO_TYPE_UINT16: {
		...
		uint16_t *r = (uint16_t *)xmalloc(n*sizeof*r);
		FORI(n)
			r[i] = .299*xd[i][0] + .587*xd[i][1] + .114*xd[i][2];
		xfree(x->data);
		x->data = r;
	...
	case IIO_TYPE_FLOAT: {
		float (*xd)[3] = x->data;
		float *r = (float *)xmalloc(n*sizeof*r);
		FORI(n)
			r[i] = .299*xd[i][0] + .587*xd[i][1] + .114*xd[i][2];
		xfree(x->data);
		x->data = r;
...
static int read_whole_tiff(struct iio_image *x, const char *filename)
{
	...
	if (TIFFIsTiled(tif)) {
		...
		uint8_t *tbuf = (uint8_t*)xmalloc(tisize*Bps*spp);
		...
		FILE *f, char *header, int nheader)
{
	...
	float *data = (float*)xmalloc(nn * sizeof*data);
		...
		FILE *f, char *header, int nheader)
{
	...
	float *data = (float*)xmalloc(w * h * 2 * sizeof(float));
...
static int read_beheaded_rim_cimage(struct iio_image *x, FILE *f, bool swp)
{
	...
	float *data = (float*)xmalloc(dx * dy);
...
static int read_beheaded_rim_fimage(struct iio_image *x, FILE *f, bool swp)
{
	...
	float *data = (float*)xmalloc(dx * dy * sizeof*data);
...
static int read_beheaded_rim_ccimage(struct iio_image *x, FILE *f, bool swp)
{
	...
	if (pm_form == 0x8001) { // ccimage
		...
		iio_wrap_image_struct_around_data(x, 2, s, pm_np, IIO_TYPE_UINT8, data);
   	        ...
   	        float*data = (float*)xmalloc(4*nsamples);
		size_t r = fread(data, 4, nsamples, f);
		if (r != nsamples) fail("rim reader failed at reading %zu "
			"samples (got only %zu)\n", (size_t)nsamples, r);
		float *good_data = (float*)xmalloc(4*nsamples);
		FORJ(pm_nrow) FORI(pm_ncol) FORL(pm_np)
			good_data[l+(i+j*pm_ncol)*pm_np] =
			data[i + j*pm_ncol + l*pm_ncol*pm_nrow];
		xfree(data); data = good_data;
		...
		FILE *f, char *header, int nheader)
{
	...
	float *data = (float*)xmalloc(w*h*4*pd);
		...
		FILE *f, char *header, int nheader)
{
	...
	float *data = (float*)xmalloc(w*h*2*sizeof*data);
		...
		FILE *f, char *header, int nheader)
{
	...
	float *u = (float*)xmalloc(w*h*sf); r = fread(u, sf, w*h, f);
	if(r != w*h) return (xfree(u),-2);
	float *v = (float*)xmalloc(w*h*sf); r = fread(v, sf, w*h, f);
	if(r != w*h) return (xfree(u),xfree(v),-2);
	float *uv = (float*)xmalloc(2*w*h*sf);
	FORI(w*h) uv[2*i] = u[i];
		...
		FILE *f, char *header, int nheader)
{
	...
	uint16_t *data = (float*)xmalloc(w*h*sizeof*data);
		...
		FILE *f, char *header, int nheader)
{
	...
	while (nheader++ < 0xf94)
		pick_char_for_sure(f);
	float *data = (float*)xmalloc(w*h*sizeof*data);
...
static int read_whole_exr(struct iio_image *x, char *filename)
{
	...
	struct ImfRgba *data = (struct ImfRgba *)xmalloc(width*height*sizeof*data);
	...
	float *finaldata = (float*)xmalloc(4*width*height*sizeof*data);
	FORI(width*height) {
		...
		FILE *f, char *header, int nheader)
{
	...
	float *xdata = (float*)xmalloc(nsamples * sizeof*xdata);
...
void iio_save_image_float_vec(char *filename, float *data, int w, int h, int pd)
{
	struct iio_image x[1];
	x->dimension = 2;
	x->sizes[0] = w;
