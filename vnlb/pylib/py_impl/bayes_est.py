...
from ..utils import groups2patches,patches2groups
...
def centering(group,nSimP,pdim,c):
...
def exec_bayes_estimate(groupNoisy,groupBasic,sigma,sigmab2,rank,nSimP,
                        channels,group_chnls,thresh,step2,mod_sel="clipped"):
    ...
    # -- group noisy --
    centerNoisy = centering(groupNoisy,nSimP,pdim,c)
    centerBasic = None
    if step2:
        centerBasic = centering(groupBasic,nSimP,pdim,c)
    # groupNoisy,groupBasic,centerNoisy,centerBasic = centering_v1(groupNoisy,groupBasic)
...
def compute_eig_stuff(group,shape,rank):
    ...
    results = pyvnlb.computeCovMat(group,rank)
